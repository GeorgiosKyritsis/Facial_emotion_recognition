...
class MetricsCallback(Callback):
    def __init__(self, training_data, validation_data):
        ...
        self.filepath = os.path.join('../../logs/VGG16', 'metrics.csv')
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.monitor_op(current, self.best):
            ...
            if self.save_weights_only:
                file = '../../logs/VGG16/' + 'weights-epoch-{0:06d}-acc_test-{1:.5f}.hdf5'.format(epoch, self.best)
                self.model.save_weights(file)
            else:
                file = '../../logs/VGG16/' + 'model-epoch-{0:06d}-acc_test-{1:.5f}.hdf5'.format(epoch, self.best)
