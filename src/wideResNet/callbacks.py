...
class MetricsCallback(Callback):
    def __init__(self, validation_data):
        ...
        self.filepath = os.path.join('../../logs/wideResNet', 'metrics.csv')
    ...
    def on_epoch_end(self, epoch, logs={}):
        ...
        if self.monitor_op(current, self.best):
            ...
            if self.save_weights_only:
                file = '../../logs/wideResNet/' + 'weights-epoch-{0:06d}-acc_test-{1:.5f}.hdf5'.format(epoch, self.best)
                self.model.save_weights(file)
            else:
                file = '../../logs/wideResNet/' + 'model-epoch-{0:06d}-acc_test-{1:.5f}.hdf5'.format(epoch, self.best)
